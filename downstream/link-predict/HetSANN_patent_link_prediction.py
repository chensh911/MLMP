...
sys.path.append("..") 
from model.HetSANN import HetSANN
...
from utils.utils import set_random_seed, load_dataset, convert_to_gpu, get_optimizer_and_lr_scheduler, \
    get_node_data_loader, get_n_params, load_patent_dataset, get_predict_edge_index, get_edge_data_loader, \
...
args = {
    'dataset': 'GoodReads',
    'model_name': 'HetSANN_link_prediction',
...
args['data_path'] = os.path.join("../../data/", args['dataset'], "model" ,f"{args['embedding_name']}.pkl")
args['data_split_idx_path'] = os.path.join("../../data/", args['dataset'], "downstream" ,"edge_split_idx.pkl")
args['truth_path'] = os.path.join("../../data/", args['dataset'], "downstream" ,"com_real_truth.json")
...
args['model_name'] = args['model_name'] + "_" + args['embedding_name']

def evaluate(model: nn.Module, loader: dgl.dataloading.DistNodeDataLoader, loss_func: nn.Module,
...
if __name__ == '__main__':
    ...
    optimizer, scheduler = get_optimizer_and_lr_scheduler(model, args['optimizer'], args['learning_rate'],
                                                          args['weight_decay'],
                                                          steps_per_epoch=len(list(train_loader)), epochs=args['epochs'])

    save_model_folder = f"../save_model/linkpred/{args['dataset']}/{args['model_name']}"
    ...
    for epoch in range(args['epochs']):
        ...
        for batch, (input_nodes, positive_graph, negative_graph, blocks) in enumerate(train_loader_tqdm):
            ...
            if args['sampled_edge_type'] == 'patent_company':
                ...
                filter_representation = {"patent": nodes_representation["patent"], "applicant": nodes_representation["applicant"]}
    ...
    # save model result
    save_result_folder = f"../results/linkpred/{args['dataset']}"
    if not os.path.exists(save_result_folder):
        os.makedirs(save_result_folder, exist_ok=True)
    save_result_path = os.path.join(save_result_folder, f"{args['dataset']}_{args['embedding_name']}_{args['model_name']}.json")
